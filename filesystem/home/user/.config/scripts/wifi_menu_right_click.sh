...
echo -e "Open Network Manager\nConnect/Disconnect Wi-Fi\nEnable/Disable Wi-Fi\nEnable/Disable Networking" | dmenu -nb '#383c4a' -nf '#ffffff' -sb '#5294e2' -fn 'DejaVu Sans:size=9.6' -p "Network Options:" | xargs -I{} sh -c 'if [ "{}" = "Open Network Manager" ]; then nm-connection-editor; elif [ "{}" = "Connect/Disconnect Wi-Fi" ]; then connect_disconnect_wifi; elif [ "{}" = "Enable/Disable Wi-Fi" ]; then toggle_wifi; elif [ "{}" = "Enable/Disable Networking" ]; then toggle_networking; fi'
